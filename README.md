...
- 카운터 saturation: `GET /api/oauth/metrics/status` 가 `{entries, maxEntries, capped, evictionsTotal, saturation}` 을 반환. cardinality 추이 모니터링용. soft cap 기본 10_000 (`OAuthMetrics({maxEntries})` 로 조정 가능).
