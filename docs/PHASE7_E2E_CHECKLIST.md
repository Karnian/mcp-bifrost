...
- [ ] **#13** Notion MCP workspace authorize 완료 후, Bifrost 콘솔에서 stream
  연결 시도 로그 확인 (`[WorkspaceManager]` 또는 providers/mcp-client.js 의
  디버그 로그). Notion 쪽에서 도구가 변경되지 않아도 최소 30초 이내 재연결
  시도가 있는지 체크.
