...
- [ ] `ws.oauth.byIdentity` 스키마 migration — 기존 `ws.oauth.tokens` → `byIdentity.default.tokens` 로 자동 이동 (부팅 시 1회)
...
- [ ] `forceRefresh(workspaceId, identity)` — per-identity mutex (`Map<"${wsId}::${identity}", Promise>`)
- [ ] `providers/mcp-client.js` — `tokenProvider` 가 callContext 의 identity 받도록 시그니처 확장 (`tokenProvider(identity)`)
- [ ] **per-workspace, per-identity warm-up 차단** — tokens 없으면 health=unknown, warmup 호출 안 함 (6-e 버그 재발 방지)
- [ ] Admin UI Detail: OAuth 패널을 `byIdentity` 탭으로 확장, 토큰 추가 버튼 → identity 선택 + authorize flow
- [ ] `/api/workspaces/:id/authorize` body 에 `{ identity }` 파라미터 추가
- [ ] 테스트: 같은 ws 에 2 identity authorize, 각각 독립 refresh, identity 해제 후 해당 tokens 만 purge, legacy 필드 migration
...
권장 실행 순서: **7b → 7a, 7d, 7f (병렬) → 7c → 7e → 7g**.

`7c` 는 `7b` 의 identity 개념이 자리잡은 후 진행해야 refactor 비용이 줄어듦. `7e` 는 `7c` 이후 진행해야 stream 도 토큰별 분리 가능.
...
### 6.1 MCP 토큰 (7b)
- 파일 저장 값은 **bcrypt 해시** — 유출돼도 직접 사용 불가
- 런타임 lookup 은 bcrypt.compare (constant-time)
...
### 6.2 ACL 강제 지점
- `tool-registry.getTools({ identity })` — tools/list 응답에서 아예 제외
- `mcp-handler` tools/call — 이중 체크 (tools/list 를 우회해 직접 호출 시도 차단)
- `resources/list`, `prompts/list` 에도 동일 필터 (누락 방지)
...
Phase 6 대비 규모 증가 (8.5 → 11일) — 멀티테넌시는 마이그레이션 리스크가 붙어 안전 마진 포함.
...
이 계획서 기준으로 Phase 7 착수 가능. 사용자 결정 필요 항목:

1. **MCP 토큰 해시 방식**: bcrypt (외부 의존성) vs node:crypto `scrypt` (의존성 0) — 기본 추천은 `scrypt`
2. **Usage JSONL 보관 기간**: 30일 기본 — 더 짧게/길게?
3. **Profile 정의 위치**: `config/workspaces.json` 내부 vs 별도 `config/profiles.json` — 기본 추천은 내부 (한 파일 관리)
4. **Phase 8 승격 후보인 항목**이 7 에도 있으면 뺄 것 (e.g. GitHub/Linear 템플릿을 별도 "community templates" 단계로)

승인 후 **7e-pre (stream probe)** → **7b (토큰 매니저 foundation)** 순서로 시작합니다.
